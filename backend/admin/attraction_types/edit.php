...
$page_title = 'تعديل نوع - Yemen Tourism';

if (!isset($_GET['id']) || empty($_GET['id'])) {
    set_flash_message('error', 'معرف النوع غير محدد');
    header('Location: ' . admin_url('attraction_types/index.php'));
    exit();
}

$id = intval($_GET['id']);
$type = db_fetch("SELECT * FROM attraction_types WHERE id = ?", [$id]);
if (!$type) {
    set_flash_message('error', 'النوع غير موجود');
    header('Location: ' . admin_url('attraction_types/index.php'));
    exit();
}
...
$form = [
    ...
    'marker_color' => $type['marker_color'] ?: '#2b8af7',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $res = db_execute("UPDATE attraction_types SET type_name = ?, description = ?, icon_name = ?, marker_color = ?, is_active = ? WHERE id = ?", [
            $form['type_name'], $form['description'], $form['icon_name'], $form['marker_color'], $form['is_active'], $id
        ]);
        if ($res !== false) {
            set_flash_message('success', 'تم تحديث النوع');
            header('Location: ' . admin_url('attraction_types/index.php'));
            exit();
        } else {
            $errors['general'] = 'فشل تحديث النوع';
        }
    }
}
            ...
            <div class="container-fluid">
                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h5 class="card-title mb-0">تعديل النوع: <?php echo htmlspecialchars($type['type_name']); ?></h5>
