...
$page_title = 'تعديل موقع - Yemen Tourism';

if (!isset($_GET['id']) || empty($_GET['id'])) {
    set_flash_message('error', 'معرف الموقع غير محدد');
...
if (!$location) {
    set_flash_message('error', 'الموقع غير موجود');
    header('Location: ' . admin_url('locations/index.php'));
    exit();
}
...
$form = [
    ...
    'zoom_level' => $location['zoom_level'] ?: 12
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $res = db_execute("UPDATE locations SET city_id = ?, name = ?, street = ?, latitude = ?, longitude = ?, zoom_level = ? WHERE id = ?", [
            $form['city_id'], $form['name'], $form['street'], $form['latitude'], $form['longitude'], $form['zoom_level'], $id
        ]);
        if ($res !== false) {
            set_flash_message('success', 'تم تحديث الموقع');
            header('Location: ' . admin_url('locations/index.php'));
            exit();
        } else {
            $errors['general'] = 'فشل تحديث الموقع';
        }
    }
}
            ...
            <div class="container-fluid">
                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h5 class="card-title mb-0">تعديل الموقع: <?php echo htmlspecialchars($location['name']); ?></h5>
